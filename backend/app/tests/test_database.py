...
class TestQueryExecutor:
    ...
    def test_dml_insert_and_delete_works(self, sqlite_engine):
        # Verify INSERT works and is committed
        res_insert = execute_query(sqlite_engine, "INSERT INTO customers VALUES (99,'Z',20)")
        assert res_insert["columns"] == ["status"]
        assert "Query executed successfully" in res_insert["rows"][0]["status"]
        
        # Verify changes are committed by doing a SELECT
        res_select = execute_query(sqlite_engine, "SELECT * FROM customers WHERE id=99")
        assert res_select["row_count"] == 1
        assert res_select["rows"][0]["name"] == "Z"

        # Verify DELETE works and returns row count
        res_delete = execute_query(sqlite_engine, "DELETE FROM customers WHERE id=99")
        assert "1 rows affected" in res_delete["rows"][0]["status"]

        # Verify changes are committed and row is gone
        res_select2 = execute_query(sqlite_engine, "SELECT * FROM customers WHERE id=99")
        assert res_select2["row_count"] == 0
